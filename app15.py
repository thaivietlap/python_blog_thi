...
def post_comment():
    ...
    if comment_content:
        comments.append({'username': username, 'content': comment_content, 'approved': False})  # Đánh dấu là chưa duyệt
        flash('Bình luận đã được đăng! Chờ duyệt.', 'success')
    else:
...
def edit_comment(comment_id):
    ...
    # Chỉ cho phép người dùng sửa bình luận của chính họ
    ...
    if request.method == 'POST':
        ...
        if new_content:
            comment['content'] = new_content  # Cập nhật nội dung bình luận
...
def approve_comment(comment_id):
    ...
    # Kiểm tra quyền duyệt bình luận
    if session.get('username') != 'admin':  # Giả sử chỉ admin có quyền duyệt
        flash("Bạn không có quyền duyệt bình luận này.", "danger")
        return redirect(url_for('show_comments'))

    comments[comment_id]['approved'] = True  # Đánh dấu bình luận đã được duyệt
    flash("Bình luận đã được duyệt!", "success")
    return redirect(url_for('show_comments'))
